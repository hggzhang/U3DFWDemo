...
int LuaAPI::NewService(lua_State *luaState) {
    ...
    //处理
    uint32_t id = Engine::inst->NewService(t);
    //返回值
...
int LuaAPI::Listen(lua_State *luaState){
    ...
    //处理
    int fd = Engine::inst->Listen(port, id);
    //返回值
...
int LuaAPI::Write(lua_State *luaState){
    ...
    int fd = lua_tointeger(luaState, 2);
    ...
    auto service = Engine::inst->GetService(service_id);
    if (service == nullptr) {
        lua_pushinteger(luaState, -1);
        return 1;
    }

    auto w = service->writers[fd];
    w->EntireWrite(shared_ptr<char>(newstr), len);
    //返回值
    lua_pushinteger(luaState, 1);
    return 1;
}