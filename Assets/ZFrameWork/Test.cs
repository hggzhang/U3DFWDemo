...
public class Test : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(500, 500, 200, 50), "检查资源更新"))
        {
            HotUpdateMgr.Inst.Init();
            HotUpdateMgr.Inst.CheckUpdate();
        }

        if (GUI.Button(new Rect(500, 700, 200, 50), "开始下载"))
        {
            HotUpdateMgr.Inst.StartDownLoad();
        }*/


        if (GUI.Button(new Rect(100, 500, 200, 50), "初始化 Net"))
        {
            NetMgr.Inst.Init();
            NetMgr.Inst.Begin();
        }
        ...
        if (!isLogin)
        {
            if (GUI.Button(new Rect(100, 700, 200, 50), "发送消息测试"))
            {
            ...
            GUI.Label(new Rect(100, 700, 200, 50), "登录成功");
    }
